...
class FKD_A(nn.Module):
    ...
    def forward(self, distiller, input_data, output_data, logging_output, batch_denom):
        ...
        logits = s_out.logits
        # Ensure labels are the same dtype and device as logits
        labels = output_data["labels"].to(dtype=logits.dtype, device=logits.device)
        ce = nn.MSELoss()(logits[:, 0], labels)  # regression loss for STS
