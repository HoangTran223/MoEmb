...
class FKD_H(nn.Module):
    ...
    def __init__(self, args):
        ...
        # Load global alignment matrix to CPU tensor
        ...
        try:
            ...
            raise RuntimeError(f"[FKD_H] Failed to load global alignment from {self.global_alignment_path}: {e}")

        # Offline projection path (W_q). If not provided, auto-assign alongside the alignment file.
    ...
    def forward(self, distiller, input_data, output_data, logging_output, batch_denom):
        ...
        # If logits is scalar per sample -> regression (MSE); otherwise classification (CE)
        if s_logits is not None and s_logits.dim() > 1 and s_logits.size(-1) == 1:
            preds = s_logits.squeeze(-1)
            # cast labels to float for MSE
            ce_loss = F.mse_loss(preds, labels_proc.to(dtype=preds.dtype))
        else:
            # classification path: ensure long dtype for labels
            ce_loss = F.cross_entropy(s_logits, labels_proc.to(dtype=torch.long))
