...
def load_model_and_tokenizer(ckpt_dir: str, device: torch.device, base_model_path: str | None = None):
    """
    ...
    if info["type"] == "full":
        ...
        config = AutoConfig.from_pretrained(ckpt_dir, trust_remote_code=False)
        model = AutoModelForSequenceClassification.from_pretrained(
            ckpt_dir, config=config, torch_dtype=torch.float32, trust_remote_code=False
        )
        model.to(device)
    else:
        ...
        model = AutoModelForSequenceClassification.from_pretrained(
            base_path, config=config, torch_dtype=torch.float32, trust_remote_code=True
        )
        # Attach adapter
...
def main():
    ...
    model, tokenizer = load_model_and_tokenizer(args.ckpt_dir, device, args.base_model_path)
