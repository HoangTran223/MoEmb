...
TEACHER_MODEL_PATH="/mnt/hungpv/projects/MoEmb/lora_path/llm2vec/patent/epoch2_step25000_loss0.1299"
NUM_LABELS=9
TASK="eaadp"
...
EAADP_ATTN_LR=0.001         # learning rate for attention weights (EAS)
EAADP_MAIN_LR=0.001          # learning rate for student + DPL + TALF (can differ from --lr scheduler)
EAADP_ALPHA=0.2          # weight for CE vs CCL: loss = alpha*CE + (1-alpha)*CCL
...
OPTS+=" --eaadp-max-cis-batches ${EAADP_MAX_CIS_BATCHES}"
if [[ ${USE_ATTENTION_CIS} -eq 1 ]]; then
    OPTS+=" --eaadp-cis-use-attn"
fi
if [[ ${USE_BATCH_CIS} -eq 1 ]]; then
    OPTS+=" --eaadp-use-batch-cis"
fi
