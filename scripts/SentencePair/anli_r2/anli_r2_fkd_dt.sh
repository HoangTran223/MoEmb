...
FKD_K=15                 # top-k teacher layers to focus on
FKD_ALPHA=0.2           # weight for CE loss
FKD_BETA=0.8             # weight for distillation loss (1 - cosine)
...
DATA="anli_r2"
CRITERION="fkd_dt" # Changed from fkd_a
SETTING=criterion=${CRITERION}__k=${FKD_K}__alpha=${FKD_ALPHA}__beta=${FKD_BETA}__epoch=${EPOCH}__bsz=${BATCH_SIZE}x${GRAD_ACC}x${GPUS_PER_NODE}=$((BATCH_SIZE * GRAD_ACC * GPUS_PER_NODE * NNODES))__lr=${LR}
