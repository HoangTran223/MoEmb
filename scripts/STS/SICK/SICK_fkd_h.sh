...
TEACHER_MODEL_PATH="/mnt/hungpv/projects/MoEmb/lora_path/llm2vec/SICK/epoch7_step7873_loss0.9858_pearson0.8734"
TASK="fkd_h"
...
FKD_H_LAMBDA=0.5     # Global-vs-contextual mix (lambda)
FKD_H_ALIGN_TOPK=10  # Top-k teacher tokens per student token
...
# Global assets live under SAVE_DIR/fkd_h
GLOBAL_DIR="${SAVE_DIR}/fkd_h"
mkdir -p "${GLOBAL_DIR}"
...
OPTS+=" --data-dir ${BASE_PATH}/dataset/data/SICK/"
OPTS+=" --num-workers 0"
OPTS+=" --dev-num 1000"
OPTS+=" --task ${TASK}"
OPTS+=" --lr ${LR}"
...
if [ ! -f "${GLOBAL_ALIGNMENT_PATH}" ] || [ ! -f "${OFFLINE_PROJ_PATH}" ]; then
  ...
  # Verify assets
  if [ ! -f "${GLOBAL_ALIGNMENT_PATH}" ] || [ ! -f "${OFFLINE_PROJ_PATH}" ]; then
    echo "[FKD_H][ERROR] Offline asset generation failed. Missing:"
    [ ! -f "${GLOBAL_ALIGNMENT_PATH}" ] && echo "  - ${GLOBAL_ALIGNMENT_PATH}"
    [ ! -f "${OFFLINE_PROJ_PATH}" ] && echo "  - ${OFFLINE_PROJ_PATH}"
    echo "Hint: ensure 'POT' (python Optimal Transport) is installed in your env (e.g., pip install POT)."
    exit 1
  fi
fi
# Run training
