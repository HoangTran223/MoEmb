...
# dataset_path = kagglehub.dataset_download('sssonnn/' + choose_dataset) # scitail, control
...
# Define column types to prevent auto-detection errors
features = Features({
    'uid': Value('string'),
...
dataset_path = f"/mnt/hungpv/projects/MoEmb/dataset/data/{choose_dataset}"
...
model = LLM2VecForSequenceClassification(num_labels=num_labels)
...
training_args = TrainingArguments(
    output_dir=f"/mnt/hungpv/projects/MoEmb/lora_path/llm2vec/{choose_dataset}",
...
trainer = CustomTrainer(
    ...
    eval_dataset=dataset["test"], # Evaluate on test set as requested
    tokenizer=tokenizer,
...
output_dir = f"/mnt/hungpv/projects/MoEmb/lora_path/llm2vec/{choose_dataset}/best_model"
print(f"Saving best model to {output_dir}")
trainer.save_model(output_dir)
tokenizer.save_pretrained(output_dir)

# The following code is for manual evaluation, which is redundant
# as trainer.evaluate already does this. It's kept here for reference.

# from torch.utils.data import DataLoader
# from tqdm import tqdm
#
# test_loader = DataLoader(dataset["test"], batch_size=32, shuffle=False, collate_fn=data_collator)
#
# all_preds = []
# all_labels = []
#
...
#
# model.save_pretrained(f"/mnt/hungpv/projects/MoEmb/lora_path/llm2vec/{choose_dataset}/best_model_manual")
...
#
# dataset_name = "llm2vec-"+ choose_dataset + "-checkpoint"  # tên sẽ xuất hiện trên Kaggle
...
# }
#
# with open(f"/mnt/hungpv/projects/MoEmb/lora_path/llm2vec/{choose_dataset}/best_model/dataset-metadata.json", "w") as f:
#     json.dump(metadata, f)
#
# get_ipython().system(f'kaggle datasets create -p /mnt/hungpv/projects/MoEmb/lora_path/llm2vec/{choose_dataset}/best_model -u')