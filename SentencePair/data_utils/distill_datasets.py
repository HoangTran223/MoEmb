...
class DistillDataset(Dataset):
    ...
    def _load_and_process_data(self):
        ...
        if os.path.exists(path):
            ...
            # Safe rank detection for standalone (non-distributed) evaluation
            try:
            ...
            for _, row in tqdm(df.iterrows(), total=len(df), disable=(rank != 0)):
                ...
                tokenized_data = {
                    ...
                    # Lưu cả premise và hypothesis gốc để dùng cho overlap
                    "premise": row['premise'],
    ...
    def collate(self, samples):
        ...
        # Thu thập text gốc cho từng sample (tuple)
        texts = [(samp["premise"], samp["hypothesis"]) for samp in samples]
        # Process each sample
        for i, samp in enumerate(samples):
            self._process_sentence_pair(i, samp, model_data, output_data)
        # Thêm text gốc vào batch để FKD_H dùng cho overlap
